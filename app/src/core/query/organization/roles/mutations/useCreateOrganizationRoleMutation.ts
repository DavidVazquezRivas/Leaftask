...
export const useCreateOrganizationRoleMutation = (organizationId: string) => {
  ...
  return useMutation({
    ...
    mutationFn: async (payload: CreateOrganizationRoleRequest) => {
      return ApiGateway.organization.roles.createRole(organizationId, payload)
    },
    onSuccess: async () => {
      await Promise.all([
      ...
      toast.success(
        i18n.t('management.settings.rolesPermissions.feedback.roleCreated', {
          ns: 'organizations',
        })
      )
    },
